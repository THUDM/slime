...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of the B positions that are response tokens. Emulates --log-probs-response-only by "
        "running CE on B*frac rows instead of B (prompt rows are gathered out before CE).",
    )
    parser.add_argument(
        "--sweep",
        action="store_true",
        help="Sweep response-frac over {1.0, 0.5, 0.25, 0.0625} at fixed total B and print a peak-memory table.",
    )
...
def _measure(B: int, args, dtype, device) -> tuple[int, int]:
    """Run CE on a [B, V] tensor and return (allocated_after_logits, peak)."""
    ...
    logits = torch.randn(B, args.vocab, device=device, dtype=dtype)
    if args.backward:
        logits.requires_grad_(True)
    tokens = torch.randint(args.vocab, (B,), device=device)
    ...
    del logits, tokens, log_probs, entropy
    return allocated_after_logits, peak
...
def main() -> None:
    ...
    print(
        f"total_B={args.batch} vocab={args.vocab} dtype={args.dtype} with_entropy={args.with_entropy} "
        f"chunk_size={args.chunk_size} backward={args.backward} mock_megatron={not args.use_real_megatron}"
    )

    if args.sweep:
        # Emulates --log-probs-response-only: CE peak should track T' = frac * T, not T.
        # frac=1.0 is the "before" (full T); smaller fracs are the gathered T'.
        print(f"{'response_frac':>14} {'T_prime':>10} {'peak':>16}")
        for frac in (1.0, 0.5, 0.25, 0.0625):
            B_eff = max(1, int(args.batch * frac))
            try:
                _, peak = _measure(B_eff, args, dtype, device)
                print(f"{frac:>14} {B_eff:>10} {_fmt_bytes(peak):>16}")
            except torch.cuda.OutOfMemoryError:
                print(f"{frac:>14} {B_eff:>10} {'OOM':>16}")
                torch.cuda.empty_cache()
        return

    B_eff = max(1, int(args.batch * args.response_frac))
    allocated_after_logits, peak = _measure(B_eff, args, dtype, device)
    print(f"response_frac={args.response_frac} -> T_prime={B_eff} (of T={args.batch})")

...
def test_dynamic_long_trajectories_ragged_samples_drop_trailing_rollouts():
    ...
    sample count and the splitter assert fired. The fix drops the minimum
    number of whole trailing rollouts so the target stays reachable, keeps
    GRPO groups atomic, and records the kept rollout count in
    ``global_batch_sizes``.
    """
    dp_size = 4
    num_rollouts = 512

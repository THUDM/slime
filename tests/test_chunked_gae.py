...
def test_gae_parallel_matches_serial(B, T, chunk_size, dtype):
    """
    ...
    rewards = torch.randn(B, T, device=device, dtype=dtype)
    values = torch.randn(B, T, device=device, dtype=dtype)
    ...
    # ---------- logging ----------
    print(f"\n[GAE Test] B={B}, T={T}, chunk={chunk_size}, dtype={dtype}")

...
# Alliance puts /cvmfs/soft.computecanada.ca/custom/python/site-packages on
# PYTHONPATH, and that dir ships a `_manylinux.py` shim. `_manylinux` is the
# hook packaging.tags consults to decide manylinux wheel compatibility; CC's
# version forces manylinux_compatible -> False so pip refuses EVERY manylinux
# wheel (steering you onto their gentoo wheelhouse). That is what makes the
# manylinux2014 torch / nvidia-* / sglang-kernel / sgl-deep-gemm wheels below
# fail with "No matching distribution found for sgl-deep-gemm==0.1.0" -- pip
# fetches the index, SEES the 0.1.0 wheel, then skips it as an incompatible
# tag. This conda env is fully self-contained, so drop the shim and let pip use
# real glibc-based manylinux detection (glibc 2.34 here -> manylinux2014 ok).
unset PYTHONPATH

# Alliance's PIP_CONFIG_FILE also injects the cvmfs wheelhouse via find-links,
# and that wheelhouse contains "dummy"/"noinstall" packages (pyarrow, mpi4py,
# cmake, opencv*, rdkit) published at version 9999+dummy.computecanada. Because
# 9999 outranks every real version, pip's resolver picks the dummy (e.g. for
# datasets' `pyarrow>=21.0.0`); the dummy then fails its build on purpose with
# "PyArrow is available from the Arrow module ... module load gcc arrow/x.y.z"
# to steer you onto `module load`. Point pip at an empty config so it ignores
# the cvmfs find-links + constraint and resolves real wheels from PyPI instead.
...
# sglang pins sgl-deep-gemm==0.1.0 / sglang-kernel==0.4.2.post2 as base deps,
# but those versions only live on the sglang wheel index (PyPI tops out at
# sgl-deep-gemm 0.0.1rc1), so add that index for the editable resolve. These
# are manylinux2014 wheels, which is why the `unset PYTHONPATH` (_manylinux
# shim) fix above is required for pip to accept them on Alliance.

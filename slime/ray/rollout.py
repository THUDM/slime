...
class RolloutManager:
    ...
    def _post_process_rewards(self, samples: list[Sample] | list[list[Sample]]):
        ...
        if (
            ...
            # rollout_id groups segments of one attempt (falls back to sample.index so the
            # default one-sample-per-rollout path is unchanged); group_index buckets rollouts
            # by prompt. Custom rollout paths may leave group_index unset -- collapse to None
            # so they keep the legacy positional grouping.
            ...
            if any(pid is None for pid in prompt_ids):
                prompt_ids = None
            std_normalization = self.args.advantage_estimator in ["grpo", "gspo"] and self.args.grpo_std_normalization

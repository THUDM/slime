...
def _compute_zero_std_metrics(args, all_samples: list[Sample]):
    ...
    def _numeric_rewards(samples: list[Sample]):
        return [r for sample in samples if isinstance((r := sample.get_reward_value(args)), (int, float))]

    def _is_zero_std(rewards: list[float]):
        return len(rewards) == 0 or all(rewards[0] == r for r in rewards)

    all_sample_groups = group_by(all_samples, lambda s: s.group_index)
    interesting_reward_groups = [
        rewards for g in all_sample_groups.values() if _is_zero_std(rewards := _numeric_rewards(g))
    ]

    interesting_rewards = [str(round(rewards[0], 1)) for rewards in interesting_reward_groups if rewards]

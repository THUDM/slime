...
from .rollout_data_source import RolloutDataSourceWithBuffer
from .utils import Lock, NOSET_VISIBLE_DEVICES_ENV_VARS_LIST, log_rollout_data, log_eval_rollout_data
...
class RolloutManager:
    def __init__(self, args, pg, wandb_run_id):
        ...
        self.generate_rollout_func = load_function(self.args.rollout_function_path)
        self.eval_generate_rollout_func = load_function(self.args.eval_function_path)
        print(f"import {self.args.rollout_function_path} as generate_rollout function.")
        print(f"import {self.args.eval_function_path} as eval_generate_rollout function.")
        
        self.custom_reward_post_process_func = load_function(self.args.custom_reward_post_process_path) if self.args.custom_reward_post_process_path is not None else None
    ...
    def generate(self, rollout_id):
        ...
        start_time = time()
        rollout_data = self._get_rollout_data(rollout_id)
        self._save_rollout_data(rollout_id, rollout_data)
        converted_data = self._convert_samples_to_train_data(rollout_data)
        log_rollout_data(rollout_id, self.args, converted_data, time() - start_time)
        return Box(ray.put(converted_data))

    def eval(self, rollout_id):
        if self.args.debug_train_only:
            # if debug train only, we don't generate evaluation data
            return
        data = self.eval_generate_rollout_func(self.args, rollout_id, self.data_source, evaluation=True)
    ...
    def _get_rollout_data(self, rollout_id):   
        if self.args.load_debug_rollout_data:
            ...
            data = self.generate_rollout_func(self.args, rollout_id, self.data_source, evaluation=False)
            # flatten the data if it is a list of lists
            if isinstance(data[0], list):
                data = sum(data, [])
        return data

    def _save_rollout_data(self, rollout_id, data):

...
The worker snapshots the current training-side ``policy_version`` at
admission and keeps it on completion. With ``max_policy_version_lag``, stale
groups return to ``data_buffer`` for regeneration from their admission-time
prompts. Aborted groups are also requeued. Dynamic sampling filters refill
rejected groups from the warm queue until ``rollout_batch_size`` is reached.
"""
            ...
            try:
                ...
                while (
                    ...
                    for group in groups:
                        ...
                        retry_group = (
                            copy.deepcopy(group)
                            if self.args.max_policy_version_lag is not None
                            else None
                        )
...
def generate_rollout_fully_async(args, rollout_id, data_buffer, evaluation: bool = False):
    ...
    metrics = worker.snapshot_metrics(reset=True) if worker is not None else {}
    if isinstance(result, RolloutFnTrainOutput):
        return RolloutFnTrainOutput(samples=result.samples, metrics={**(result.metrics or {}), **metrics})
    return RolloutFnTrainOutput(samples=result, metrics=metrics)

...
def normalize_instruction_ids(raw_ids: Any) -> list[str]:
    output: list[str] = []
...
def normalize_kwargs(raw_kwargs: Any, n: int) -> list[dict[str, Any]]:
...
def check_instruction(instruction_id: str, rule_kwargs: dict[str, Any], prompt: str, response: str) -> bool:
...
def instruction_rule_scores(metadata: dict[str, Any], prompt_text: str, response: str) -> list[float]:
    ...
    for instruction_id, rule_kwargs in zip(instruction_ids, kwargs_list):
        try:
            passed.append(1.0 if check_instruction(instruction_id, rule_kwargs, prompt_text, response) else 0.0)
        except Exception:
            passed.append(0.0)
    return passed


def compute_instruction_following_reward(
    metadata: dict[str, Any],
    prompt_text: str,
    response: str,
    *,
    ifbench_rule_scores_fn=None,
    ifeval_rule_scores_fn=None,
) -> float:
    reward_type = str(metadata.get("reward_type", "")).strip()
    if reward_type not in {"instruction_following_soft", "instruction_following_strict", "ifeval"}:
        return 0.0
    dataset_name = str(metadata.get("dataset_name", "")).strip().lower()
    if dataset_name == "ifbench_test":
        if ifbench_rule_scores_fn is None:
            raise RuntimeError("IFBench official evaluation_lib is required for ifbench_test.")
        scores = ifbench_rule_scores_fn(metadata, prompt_text, response, reward_type != "instruction_following_soft")
        if scores is None:
            raise RuntimeError(
        ...
        if ifeval_rule_scores_fn is None:
            raise RuntimeError("Official IFEval backend is required for ifeval.")
        scores = ifeval_rule_scores_fn(metadata, prompt_text, response, reward_type != "instruction_following_soft")
        if scores is None:
            raise RuntimeError(

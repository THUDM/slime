...
async def generate_rollout_async(args, rollout_id: int, data_source) -> tuple[list[list[Sample]], list[list[Sample]]]:
    ...
    Returns:
        tuple[list[list[Sample]], list[list[Sample]]]:
            - data: a list of samples generated by the rollout, length equals `rollout_batch_size`
            - aborted_samples: any partial groups collected during abort when partial_rollout is enabled
        tuple[list[list[Sample]], list[list[Sample]]]:
            - completed samples grouped per prompt

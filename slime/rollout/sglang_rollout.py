...
from slime.utils.url_utils import get_default_router_url_from_args, get_model_url_from_args
...
async def abort(args: Namespace, rollout_id: int) -> list[list[Sample]]:
    ...
    try:
        ...
        if not getattr(args, "rollout_external", False) or getattr(response, "status_code", None) != 404:
            raise
        urls = [get_default_router_url_from_args(args, "/").rstrip("/")]
        logger.info("Router workers endpoint is unavailable for external direct engine; aborting via base URL.")

...
async def generate_and_rm_group(
    ...
    if not state.aborted and args.group_rm:
        reward_samples = [sample for result in group for sample in (result if isinstance(result, list) else [result])]
        with trace_span(reward_samples, "group_reward_model"):
            rewards = await batched_async_rm(args, reward_samples)
        for sample, reward in zip(reward_samples, rewards, strict=True):
            sample.reward = reward

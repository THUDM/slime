"""Test-internal compact-rollout helper used by ``test_qwen2.5_0.5B_fanout_short.py``.
...
The function fans one input sample out to ``1 + (index % MAX_FANOUT)``
training samples sharing the same ``rollout_id`` — that's the contract
the rest of the framework (per-rollout step splitter, per-rollout-mean
reducer, ``_validate_rollout_id_annotated`` validator) is built around.
"""

...
# Modified by the slime project in 2026.
# Changes support non-contiguous upstream gradients from slime's pipeline output.
...
class LinearCrossEntropy(torch.autograd.Function):
    ...
    def backward(ctx, dlogprobs: torch.Tensor, dentropy: torch.Tensor) -> list[torch.Tensor]:
        ...
        with torch.cuda.nvtx.range("LinearCrossEntropy-backward"):
            # Slime packs both outputs into one pipeline tensor. Unbinding that
            # tensor produces strided upstream gradients, while the Triton host
            # API requires contiguous vectors.

...
def get_chunked_loss_masks(
    total_lengths: list[int], response_lengths: list[int], loss_masks: list[torch.Tensor]
...
def get_sum_of_sample_mean(
    ...
    """
    cp_size = mpu.get_context_parallel_world_size()
    chunked_loss_masks, chunk_lengths = get_chunked_loss_masks(total_lengths, response_lengths, loss_masks)
            ...
            return sum(
                [
                    ...
                    for x_i, chunked_loss_mask, loss_mask in zip(
                        x.split(chunk_lengths, dim=0), chunked_loss_masks, loss_masks, strict=False
                    )
                ]
            )
        ...
        def sum_of_token(x: torch.Tensor) -> torch.Tensor:
            return sum(
                [
                    ...
                    for x_i, chunked_loss_mask in zip(x.split(chunk_lengths, dim=0), chunked_loss_masks, strict=False)
                ]
            )

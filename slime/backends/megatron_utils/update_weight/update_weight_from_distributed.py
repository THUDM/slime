...
class UpdateWeightFromDistributed:
    ...
    Subclasses override ``_send_weights`` to inject per-mode behaviour (e.g. delta).
    """
    ...
    def _send_weights(self, pbar: tqdm | None) -> None:
        """
        Non-expert (TP) loop → barrier → expert (EP) loop. Subclasses should
        override ``_on_chunk`` to extend; override this only when the bucketing
        strategy differs.
        """
        buffer_size = 0
        ...
        for hf_chunk in self._iter_non_expert_chunks():
            ...
            if buffer_size + chunk_bytes > self.args.update_weight_buffer_size:
                self._update_bucket_weights_from_distributed(buffer, pbar=pbar)
                buffer = []
                buffer_size = 0
    ...
    def _iter_non_expert_chunks(self) -> Iterator[list[tuple[str, torch.Tensor]]]:
        """
        Yield one HF chunk per non-expert param after TP all-gather + HF convert.
        Empty generator on non-PP-src ranks (they still participate in all_gather_param).
        """
        for name, param in named_params_and_buffers(self.args, self.model):
            ...
            if not self._is_pp_src_rank:
                continue
            yield convert_to_hf(self.args, self.model_name, name, param, self.quantization_config)

    def _iter_expert_chunks(self) -> Iterator[list[tuple[str, torch.Tensor]]]:
        """
        Yield one HF chunk per EP-weighted flush of expert params: TP gather + buffer
        until threshold, then EP gather + HF convert. All ranks participate in the EP
        collective; only PP source yields a non-empty chunk and only on those ranks
        does the generator advance.
        """
        buffer_size = 0
    ...
    def _ep_gather_and_convert(self, named_tensors: list[tuple[str, torch.Tensor]]) -> list[tuple[str, torch.Tensor]]:
        """
        EP all-gather a buffered batch + HF convert on PP source. Clears ``named_tensors``.
        Returns HF tensors on PP source, [] elsewhere.
        """

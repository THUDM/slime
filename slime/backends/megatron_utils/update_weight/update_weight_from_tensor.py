...
class UpdateWeightFromTensor:
    ...
    def update_weights(self) -> None:
        ...
        if self.delta_tracker is None:
            for hf_named_tensors in self._hf_weight_iterator.get_hf_weight_chunks(megatron_local_weights):
                chunk_update = self._prepare_hf_chunk_for_send(hf_named_tensors)
                ...
                refs, long_lived_tensors = self._send_hf_params(
                    chunk_update.tensors, load_format=chunk_update.load_format
                )
                ray.get(refs)
                self._finalize_sent_chunk(chunk_update.commit_state)
            ...
            for hf_named_tensors in self._hf_weight_iterator.get_hf_weight_chunks(megatron_local_weights):
                ...
                if pending_bucket.should_flush_before_add(chunk_update, self.args.update_weight_buffer_size):
                    self._flush_hf_update_bucket(pending_bucket)
    ...
    def _prepare_hf_chunk_for_send(
        ...
        if self.delta_tracker is None:
            return HFUpdate(tensors=list(hf_named_tensors), load_format="flattened_bucket", commit_state=None)

        prepared = self.delta_tracker.prepare_chunk(hf_named_tensors)
        load_format = "flattened_bucket_delta" if prepared.is_delta else "flattened_bucket"
        return HFUpdate(tensors=prepared.tensors, load_format=load_format, commit_state=prepared.commit_state)
    ...
    def _flush_hf_update_bucket(self, pending_bucket: PendingHFUpdateBucket) -> None:
        if not pending_bucket.has_updates:
            return

        refs, long_lived_tensors = self._send_hf_params(pending_bucket.tensors, load_format=pending_bucket.load_format)
        ray.get(refs)
    ...
    def _send_hf_params(self, hf_named_tensors, load_format: str = "flattened_bucket") -> tuple[list[ObjectRef], Any]:
        all_refs = []

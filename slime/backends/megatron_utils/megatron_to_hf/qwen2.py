...
def convert_qwen2_to_hf_fp8(args, name, param):
    if name == "module.module.embedding.word_embeddings.weight":
        return [("model.embed_tokens.weight", param)]
    if name == "module.module.output_layer.weight":
        return [("lm_head.weight", param)]
    ...
    if match:
        layer_idx, rest = match.groups()
        if rest == "self_attention.linear_proj.weight.fp8_weight":
            return [(f"model.layers.{layer_idx}.self_attn.o_proj.weight", param)]
        elif rest == "self_attention.linear_proj.weight.fp8_scale":
            return [(f"model.layers.{layer_idx}.self_attn.o_proj.weight_scale_inv", param)]
        elif rest == "self_attention.linear_qkv.weight.fp8_weight":
        ...
        elif rest == "self_attention.linear_qkv.weight.fp8_scale":
            hidden_size_scale = math.ceil(args.hidden_size / 128)
            param = param.view(args.num_query_groups, -1, hidden_size_scale)
            q_param, k_param, v_param = torch.split(param, split_size_or_sections=[value_num_per_group, 1, 1], dim=1)
            q_param = q_param.reshape(-1, hidden_size_scale)
            k_param = k_param.reshape(-1, hidden_size_scale)
            v_param = v_param.reshape(-1, hidden_size_scale)
            return [
                (f"model.layers.{layer_idx}.self_attn.q_proj.weight_scale_inv", q_param),
                (f"model.layers.{layer_idx}.self_attn.k_proj.weight_scale_inv", k_param),
                (f"model.layers.{layer_idx}.self_attn.v_proj.weight_scale_inv", v_param),
            ]
        elif rest == "self_attention.linear_qkv.bias":
            param = param.view(args.num_query_groups, -1)
            q_bias, k_bias, v_bias = torch.split(
        ...
        elif rest == "mlp.linear_fc1.weight.fp8_weight":
            gate_weight, up_weight = param.chunk(2, dim=0)
            return [
                (f"model.layers.{layer_idx}.mlp.gate_proj.weight", gate_weight),
                (f"model.layers.{layer_idx}.mlp.up_proj.weight", up_weight),
            ...
            return [(f"model.layers.{layer_idx}.mlp.down_proj.weight", param)]
        elif rest == "mlp.linear_fc2.weight.fp8_scale":
            return [(f"model.layers.{layer_idx}.mlp.down_proj.weight_scale_inv", param)]
        elif rest == "self_attention.linear_qkv.layer_norm_weight":
            return [(f"model.layers.{layer_idx}.input_layernorm.weight", param)]
        elif rest == "mlp.linear_fc1.layer_norm_weight":
            return [(f"model.layers.{layer_idx}.post_attention_layernorm.weight", param)]

    ...
    def add_slime_arguments(parser):
        ...
        def add_train_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Path to a custom function called inside each rollout engine actor before "
                    "forwarding a delta ``update_weights_from_disk`` to the SGLang HTTP server. "
                    "Signature: ``def hook(engine) -> None``. Once per engine actor (one per "
                    "container); used for non-POSIX shared FS to reload the engine's view of "
                    "trainer-published files."
                ),
            )
            parser.add_argument(

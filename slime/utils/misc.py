...
def should_run_periodic_action(
    ...
    *,
    ...
    Args:
        ...
        num_rollout: Optional total rollout count used to detect the final step.
        skip_final_step: Whether to skip the final step even when another trigger matches.
    """
    if interval is None:
        return False

    if num_rollout is not None and rollout_id == num_rollout - 1:
        return not skip_final_step

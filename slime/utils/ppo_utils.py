...
def compute_reinforce_loss(
    ...
    """REINFORCE surrogate ``-A * log pi_theta`` (no IS ratio, no clipping).

    Gradient flows only through ``log_probs``. Same ``(per_token_loss, clipfrac)``
    contract as :func:`compute_policy_loss`, with ``clipfrac`` identically zero.
    """

...
def calculate_log_probs_and_entropy_fused(hidden, weight, tokens, tp_group, with_entropy: bool = False, chunk_size: int = -1):
    ...
    def _chunk_fn(hidden_chunk, token_chunk, lm_head_weight):
        ...
        entropy_chunk = compute_entropy_from_logits(logits_chunk.clone(), tp_group) if with_entropy else None
        log_prob_chunk = compute_log_probs(logits_chunk, token_chunk, tp_group)
        return log_prob_chunk, entropy_chunk

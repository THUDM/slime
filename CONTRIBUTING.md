...
slime is the RL training infrastructure behind [GLM-4.5 through GLM-4.7](https://z.ai) and a large number of internal experiments at Zhipu AI. We open-sourced slime because we believe the training scenarios used internally cover the majority of cutting-edge RL algorithm requirements, and we hope to provide the community with a correct and efficient large-scale RL training infrastructure.
...
| Category | Reason |
|----------|--------|
| **Large-scale code refactoring** | Significantly increases the burden of syncing between the internal and open-source versions, especially in communication with internal algorithm teams. |
...
| **Features that cannot be clearly verified** | Correctness is the lifeline of a training framework. If a feature cannot be verified through CI or routine internal training, we cannot ensure timely fixes, which poses a serious risk to the project's reputation. |
...
slime's design and development roadmap must first align with Zhipu AI's internal requirements — the RL infrastructure design is tightly coupled with post-training plans, and publishing the full post-training R&D roadmap is not within the open-source scope of slime. This focused scope is what we believe is necessary to maintain slime as a long-term, trustworthy project.

We understand this may slow down slime's pace of feature development. We are actively expanding the slime team through hiring to address this — if you're interested, feel free to reach out directly.

Thank you for your understanding. We sincerely apologize for any inconvenience this policy may cause to community contributors who have invested their efforts.
...
slime 承担了智谱内部的大量实验，包括 GLM 4.5 至 4.7 的全部 RL 流程，以及大量的日常实验。我们开源的初衷是相信智谱内部的训练场景覆盖了大多数前沿算法需求，希望能够为社区提供一套**正确且高效**的大规模 RL 训练 Infra，同时也希望在此基础上和社区进行 Infra 性能优化上的共建。

在我们的已知范围内，目前只有极少的前沿大模型团队愿意公开如此核心且完整的 Infra 组件——这背后是公司对开源社区的极大热情。相应的，从维护者的角度，我们会充分利用这个开明的政策，让智谱内部使用的 slime 与开源版本同步，保持核心竞争力；同时，我们也不能因为开源而拖慢了内部的研发节奏。这意味着 slime 的设计与开发 roadmap 需要首先参考智谱内部的需求，而不能完全在开源社区内进行讨论。
...
| 类别 | 原因 |
|------|------|
| **较大范围的代码重构** | 会给内外部同步带来比较大的负担，尤其是和内部算法团队的交流负担。 |
...
| **无法进行明确验证的功能** | 训练框架的正确性是生死线。如果一个功能不能通过 CI 或智谱内部常规训练进行验证，我们就无法及时修复，对项目的名誉是个比较严重的打击。 |
| **与 RL 框架较为独立的功能** | 例如整套算法复现流程。这样的流程很难进入日常的验证流程中，很难保证正确性。slime 是一个相对轻量的框架，更像是 Flask 而非 Django。建议通过搭建独立的 repo 进行，我们也愿意在 README 中引用所有使用了 slime 的项目链接。 |
...
将上述 feature 拒之门外也许会减慢 slime 的开发速度，我们会通过招聘的方式逐渐扩展 slime 团队来进行弥补——对此有兴趣的朋友可以直接私信联系~

感谢大家的理解，对受影响的社区朋友表示街心的歉意。

#!/usr/bin/env bash
# End-to-end SWE RL case: default linear Sample payload, no trajectory_tree.

set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
export SWE_SAVE_TRAJECTORY_TREE=0
export RUN_ROOT="${RUN_ROOT:-/mnt/jingshenghang/storage/slime_swe_runs/qwen36_cagent_linear_e2e_$(date +%Y%m%d_%H%M%S)}"

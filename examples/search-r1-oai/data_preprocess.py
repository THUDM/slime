...
# Adapted from: https://github.com/volcengine/verl/blob/main/examples/data_preprocess/preprocess_search_r1_dataset.py
#
...
SEARCH_TOOL =  {
    ...
    "function": {
        "name": "search",
        "description": (
            "Search for relevant documents using dense retrieval with e5 embeddings and FAISS indexing. "
            "This tool performs semantic search over a corpus of documents using the e5-base-v2 encoder "
            "and returns the most relevant passages based on embedding similarity. "
            "Use this to find factual information or context to answer questions."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "queries": {
                    ...
                    "description": (
                        "List of search queries to find relevant documents. Each query should be a "
                        "natural language question or keyword phrase. The API supports batch retrieval."
                    ),
                    "minItems": 1
                },
                # "topk": {
                #     "type": "integer",
                #     "description": (
                #         "Number of top relevant documents to retrieve per query. "
                #         "If not specified, uses the server's default (typically 3). "
                ...
                #     "type": "boolean",
                #     "description": (
                #         "Whether to return similarity scores alongside documents. "
                #         "If true, returns {\"document\": {...}, \"score\": float} objects. "
                #         "If false (default), returns only document objects."
                #     ),
                #     "default": False
                # }
            },
    ...
    return pd.Series(
        {
            ...
            "prompt": prompt,
            "tools": json.dumps([SEARCH_TOOL]),  # Serialize to JSON string to avoid numpy conversion in parquet
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_dir",
        default="data/search_R1_processed",

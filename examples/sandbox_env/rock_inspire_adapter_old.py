...
class InspireRockSandbox:
    ...
    @staticmethod
    def _shell_quote_env(env: dict[str, str] | None) -> str:
        if not env:
            return ""
        return " && ".join(f"export {str(k)}={shlex.quote(str(v))}" for k, v in env.items())
    ...
    def _wrap_shell(
        ...
        *,
        ...
        parts: list[str] = []
        env_exports = self._shell_quote_env(env)
        if env_exports:
    ...
    async def run_in_session(self, action: Action) -> Observation:
        state = self._sessions.get(action.session, _SessionState(env={}))
        remote_user = state.remote_user or DEFAULT_SESSION_USER
        wrapped_cmd = self._wrap_shell(action.command, cwd=state.cwd, env=state.env)
        self._append_live_log(
        ...
        inner = f"{wrapped_cmd}; __rc=$?; pwd > {shlex.quote(cwd_tmp)} 2>/dev/null; exit $__rc"
    ...
    async def arun(
        ...
        if mode == RunMode.NORMAL:
            if session is None:
                wrapped_cmd = self._wrap_shell(
                    cmd,
                    env=await self._build_command_env(user=command_user),
                )
                return await self._run_command(wrapped_cmd, timeout=wait_timeout, user=command_user)
    ...
    async def _arun_nohup(
        ...
        *,
        ...
        if session is None:
            run_user = user or DEFAULT_SANDBOX_USER
            wrapped_cmd = self._wrap_shell(
                cmd,
                env=await self._build_command_env(user=run_user),
            )
        else:
            state = self._sessions.get(session, _SessionState(env={}))
            run_user = state.remote_user or DEFAULT_SESSION_USER
            wrapped_cmd = self._wrap_shell(cmd, cwd=state.cwd, env=state.env)

        self._append_live_log(
            f"[nohup.start] session={session} wait_timeout={wait_timeout} wait_interval={wait_interval} "
            f"ignore_output={ignore_output} output_file={tmp_file} command={wrapped_cmd}"
        )
        ...
        nohup_command = (
            f"nohup {wrapped_cmd} < /dev/null > {shlex.quote(tmp_file)} 2>&1 "
            f"& echo {PID_PREFIX}${{!}}{PID_SUFFIX};disown"
        )
        self._append_live_log(
            f"[nohup.submit] user={run_user} output_file={tmp_file} "
            f"command={wrapped_cmd} > {shlex.quote(tmp_file)} 2>&1"
        )
        submit_result = await self._run_command(nohup_command, timeout=30, user=run_user)
        if submit_result.exit_code != 0:
            return submit_result

        pid = extract_nohup_pid(submit_result.output or "")
        if not pid:
            self._append_live_log(f"[nohup.pid_missing] output_file={tmp_file}")
            return Observation(
                output="Failed to submit command, nohup failed to extract PID",
                exit_code=1,
                failure_reason="Failed to submit command, nohup failed to extract PID",
            )
        ...
        read_back = await self._run_command(cat_cmd, timeout=60, user=run_user)
        self._append_live_log(
    ...
    async def start_nohup_process(self, cmd: str, tmp_file: str, session: str) -> tuple[int | None, Observation | None]:
        ...
        self._append_live_log(
            f"[start_nohup_process.submit] session={session} output_file={tmp_file} "
            f"command={wrapped_cmd} > {shlex.quote(tmp_file)} 2>&1"
        )
        response = await self.run_in_session(Action(command=nohup_command, session=session, timeout=30))
        if response.exit_code != 0:
            return None, response

        pid = extract_nohup_pid(response.output or "")
        if not pid:
            self._append_live_log(
    ...
    async def wait_for_process_completion(
        ...
        return await self._wait_for_process_completion(
            ...
            wait_timeout=wait_timeout,
            wait_interval=wait_interval,
            user=(self._sessions.get(session, _SessionState(env={})).remote_user or DEFAULT_SESSION_USER),
        )
    ...
    async def handle_nohup_output(
        ...
        if ignore_output:
            file_size = None
            try:
                size_result = await self.run_in_session(
                    Action(
                        command=f"stat -c %s {shlex.quote(tmp_file)} 2>/dev/null || stat -f %z {shlex.quote(tmp_file)}",
                        session=session,
                        timeout=30,
                    )
                )
                if size_result.exit_code == 0 and (size_result.output or "").strip().isdigit():
                    file_size = int((size_result.output or "").strip())
            except Exception:
                pass
            return Observation(
                output=self._build_nohup_detached_message(tmp_file, success, message, file_size),
    ...
    async def _run_command(
        ...
        *,
        ...
        self._append_live_log(
            f"[command.start] user={log_user} timeout={timeout} command={command}"
        )

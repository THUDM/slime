#!/bin/bash
# 5-harness G_C2 spec validation smoke (Qwen3.5-35B-A3B, single node, 8 GPUs).
#
...
#
# Mirrors `_run_5harness_gc2_smoke_orchestrator.sh` but uses the 35B model
# (4 actor + 4 rollout GPUs).
#
# Inputs (must already exist):
#   data_output/swe_rebench_tasks_smoke_gc2_3rows.json
#   data_output/swe_rebench_scaffold_template_smoke_gc2.jsonl
#   templates `swe-rebench-gc2-{slug}` already built
#
# Outputs:
#   output/qwen3_5_35b_smoke_gc2/orchestrator_5harness_35b_gc2.log
#   output/qwen3_5_35b_smoke_gc2/qwen3.5-35b-smoke-gc2-{harness}.log
#   output/qwen3_5_35b_smoke_gc2/smoke-35b-gc2-{harness}/...
set -uo pipefail
...
LOG_DIR="${SANDBOX_ENV_DIR}/output/qwen3_5_35b_smoke_gc2"
mkdir -p "${LOG_DIR}"

ORCH_LOG="${LOG_DIR}/orchestrator_5harness_35b_gc2.log"
...
for h in "${HARNESSES[@]}"; do
  log="${LOG_DIR}/qwen3.5-35b-smoke-gc2-${h}.log"
  ...
  SWE_WANDB_GROUP="smoke-35b-gc2" \
  SWE_WANDB_RUN_ID="qwen3.5-35b-smoke-gc2-${h}" \

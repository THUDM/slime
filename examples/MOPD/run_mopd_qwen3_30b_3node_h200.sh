...
SYSTEM_QUERIES_PY="${SLIME_DIR}/examples/common/system_queries.py"
...
load_eval_data_specs() {
  mapfile -t EVAL_DATA_SPECS < <(
    python3 "${DATASET_QUERIES_PY}" load-eval-config --path "${EVAL_CONFIG_PATH}"
  )

  if [[ "${TEACHER_STEP0_INCLUDE_BFCL}" == "1" ]] && [[ -f "${OFFICIAL_EVAL_MANIFEST_PATH}" ]]; then
    mapfile -t official_specs < <(
      python3 "${DATASET_QUERIES_PY}" load-json-manifest --path "${OFFICIAL_EVAL_MANIFEST_PATH}"
    )
...
run_teacher_step0_eval() {
  ...
  run_id="$(python3 "${SYSTEM_QUERIES_PY}" random-run-id)"

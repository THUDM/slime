...
| 12 | `sglang_router` x86_64-only wheel from `zhuzilin/sgl-router` fork | slime Dockerfile pins `zhuzilin/sgl-router` release (no arm64 builds); SGLang Prometheus metrics are now exported for Prometheus scraping instead of W&B upload | Install upstream `sglang-router==0.3.2` from PyPI (has arm64 wheel) |

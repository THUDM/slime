...
| slime 负责启动 SGLang engine，并希望使用 Mooncake TransferEngine 做全量权重同步 | [Mooncake TransferEngine 权重同步](mooncake-weight-sync.md) |
...
- Mooncake TransferEngine 权重同步当前只支持 slime-managed engine；与 `--rollout-external-engine-addrs` 同用会被拒绝。

...
3. **为什么训着训着 OOM 了？`max_tokens_per_gpu` 是干什么用的？**
...
4. **多机训练的时候，遇到了 transformers 库找不到某个模型的错误该怎么办？**

...
- `--score-centering-top-k`: Number of sampler top-k token IDs and logprobs retained for each response token; defaults to 128 and must fit the model vocabulary. The head retains its full-vocabulary probability mass. The remaining sampler mass is modeled as proportional to the current trainer's tail mass.
...
**Sampling requirements:** Use a positive temperature, `top_p=1`, `top_k=-1`, `min_p=0`, and no repetition/frequency/presence penalties or constrained decoding. Keep `SGLANG_RETURN_ORIGINAL_LOGPROB` unset or false on all sampler workers when temperature differs from one. Per-request temperature changes and streaming SC are unsupported. Evaluation does not request SC data and may use its own sampling settings.

**SGLang support:** Use an image built with `docker/patch/latest/sglang-top_p.patch`, which includes binary top-k output alongside top-p replay. Slime requests `top_logprobs_num=k` and stores the original sampler probabilities, rather than recomputing them with a newer checkpoint. Custom generators should call `score_centering_request` from `slime.utils.score_centering` and pass the response metadata to `Sample.append_response_tokens`.

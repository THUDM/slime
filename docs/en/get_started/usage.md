...
Note:
  ...
  - For slime-managed SGLang engines, full-weight sync can also use Mooncake TransferEngine with `--update-weight-mode full --update-weight-transport mooncake`. This path currently requires `--mooncake-metadata-server P2PHANDSHAKE`, one GPU per rollout engine, no `--colocate`, no external rollout engines, and `--mooncake-buffer-size` large enough for the largest weight bucket. See [Mooncake TransferEngine Weight Sync](../advanced/mooncake-weight-sync.md).

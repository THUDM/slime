...
**Reasoning-fork policy (`SLIME_AGENT_REASONING_FORK_POLICY`, default `off`).** Agent clients disagree on how they echo `reasoning_content` back in later prompts -- most echo it verbatim or drop it. When a replayed assistant message no longer matches the recorded turn, a short generated turn is normally rewrite-merged into routing-only context and stops training. `reasoning_dropped` carves out the common drop case: whenever the recorded turn carried `reasoning_content` and the echo dropped it, the trajectory forks instead, keeping the generated turn trainable -- regardless of response length and of whether other fields also changed. Anything beyond a drop (e.g. the client rewrote the reasoning) gets no carve-out and falls back to the threshold merge/fork logic -- merged away and demoted when short (a rewritten reasoning is treated as no longer worth training), forked as usual when long. `off` disables the carve-out entirely. **Cost note:** each fork splits one rollout into more training samples, so enabling the policy raises sample count and training cost -- enable it only when the client's echo behavior would otherwise silently demote trained turns. On the OpenAI adapter the policy engages only together with `SLIME_AGENT_OPENAI_MANAGER_REASONING`: with the manager switch off, the manager message and the dropped echo both omit `reasoning_content`, history still matches, and the loss happens at the token level, out of the policy's reach. On the Anthropic adapter (whose manager message always carries `reasoning_content`) the policy works on its own.

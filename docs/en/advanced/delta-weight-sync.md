...
- **Publish overlaps training.** The dispatched publish stays in flight across the next training step and is settled at the start of the next sync (or on disconnect). A failed publish therefore surfaces one sync late, on rank 0.
- **Engines are left alone.** Generation is not paused, caches are not flushed, and no update RPCs are issued; consumers decide when to pick up a version. Pair with `--rollout-weight-version-policy exact-rollout-id` if requests must wait for a specific version.
